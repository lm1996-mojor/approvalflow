...
type PanelPoint struct {
	...
	OnlyWebUsePreviousStep int64  `json:"onlyWebUsePreviousStep,omitempty"` // 上一节点id（仅限前端遍历使用）
}

// 定义列
func (c *PanelPoint) allColumn() []string {
	columns := []string{"process_id", "point_name", "point_code",
		"scenario", "point_type", "examine_type", "priority", "condition_type", "only_web_use_previous_step"}
