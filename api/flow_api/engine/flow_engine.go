...
/*
	...
	|———————————————————|———————————————————————|———————————————————————————————————————————————————————————————————————|————————————————————————————————————————————————————————————————————————————————————————|
	| ProcessRate		|	流程审批进度   	    |（1 同意 2退回 3驳回 4审批中 5待执行 6撤销）									|		/																				 |
...
func StartFlow(path string) {
	// 开始执行流程
	approvalParams, err := file_util.ReaderJsonFileToObj(path, _const.APPROVAL_DATA_FILE_SAVE_FILENAME, api_model.ApprovalParams{})
	if err != nil {
	...
	approvalParams.PointDetails = ApprovalPointChangeFun(approvalParams.PointDetails, promoterPointDetail)
	// 流程进入到下一个审批节点
	approvalParams.PointDetails = NextStepPoint(approvalParams.PointDetails, promoterPointDetail)
